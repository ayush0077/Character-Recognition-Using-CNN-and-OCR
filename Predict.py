...
def Predict(Characters, Models, Label_Dict, Evaluate=False):
    Predictions = []
    Model_Predictions = []

    for CharactersSet in Characters:
        Prediction = []
        Model_Prediction = []
        ...
        for Character in CharactersSet:
            ...
            if np.isscalar(mode_result.mode):
                ...
                mode_value = mode_result.mode[0][0] if mode_result.count[0] > 0 else None

            if mode_value is not None:
                Prediction.append(str(Label_Dict[mode_value]))
            else:
                # Handle the case where there is no mode (all elements in y are unique)
                Prediction.append("No mode found")
